...
async def ro_prcart_get_all(user_id:int=Depends(get_user_id), 
...
async def ro_prcart_get_id(pro_id:int, user_id:int=Depends(get_user_id), 
    ...
    return await CartService.se_cart_get_id(db, user_id, pro_id)
...
async def ro_prcart_create(prcart:PrCartCreate,
                       ...
                       db:AsyncSession=Depends(get_db)):
    return await CartService.se_add_or_update_product(db, user_id, prcart)

# 상품 
@router.put("/{pro_id}", response_model=PrCartRead, status_code=status.HTTP_200_OK)
async def ro_prcart_update(prcart:PrCartUpdate,
                           pro_id: int,
                           user_id:int=Depends(get_user_id),
                           db:AsyncSession=Depends(get_db)):
    return await CartService.se_prcart_update(db, prcart, pro_id)

# 상품 삭제
@router.delete("/{pro_id}", response_model=PrCartRead, status_code=status.HTTP_200_OK)
async def ro_prcart_delete(pro_id:int,
                       user_id:int=Depends(get_user_id),
                       db:AsyncSession=Depends(get_db)):
    return await CartService.se_prcart_delete(db, pro_id)

@router.delete("/delete", response_model=PrCartRead, status_code=status.HTTP_200_OK)
async def ro_ca_delete(db:AsyncSession=Depends(get_db)):
    return await CartService.se_ca_delete(db)
