...
def create_access_token(uid:int, **kwargs)->str:
    return create_token(uid=uid, expires_delta_seconds=settings.access_token_expire_seconds,**kwargs)
...
def create_refresh_token(uid:int) -> str:
    return create_token(uid=uid, jti=str(uuid.uuid4()), expires_delta_seconds=settings.refresh_token_expire_seconds)
